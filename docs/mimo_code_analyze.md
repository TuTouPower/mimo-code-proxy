...
JWT payload 含 `exp` 字段（Unix 秒）。每次发起 LLM 请求前检查剩余时间：若 `exp * 1000 - Date.now() > 300000`（即距离过期超过 5 分钟）则复用现有 token，不做额外请求；仅在剩余有效时间不足 5 分钟时，才在 chat 请求前自动调一次 bootstrap 续期。收到 401/403 时无视剩余时间立即强制刷新。
...
所有以下 header 都会被 `mimo-free` 自定义 fetch 保留并原样透传到服务端（`new Headers(req.headers)` 全量复制后追加 auth header）。

| Header | 值 | 代码来源 | 说明 |
|--------|-----|----------|------|
| `Authorization` | `Bearer {jwt}` | mimo-free 自定义 fetch | 每次 chat 请求必带 |
| `X-Mimo-Source` | `mimocode-cli-free` | mimo-free 自定义 fetch | 每次 chat 请求必带 |
| `Content-Type` | `application/json` | AI SDK | 每次必带 |
| `User-Agent` | `mimocode/{channel}/{version}/cli` | `session/llm.ts:607` | 每次必带 |
| `x-session-affinity` | `{sessionID}` | `session/llm.ts:603` | 每次必带，value 为当前会话 ID |
| `x-parent-session-id` | `{parentSessionID}` | `session/llm.ts:604` | **仅子会话（subagent）场景携带**。普通 `mimo run` 对话中 `parentSessionID` 为 `undefined`，此 header 不发送 |
...
| `stream` | `true` | AI SDK `doStream()` 硬编码，始终发送 |
| `stream_options` | `{"include_usage": true}` | `provider.ts:1441-1443` → `includeUsage: true` → AI SDK 输出 |
| `messages` | 见下方 | `session/llm.ts:362-370` + `message-v2.ts:toModelMessages()` |
| `tools` | 见下方 | `session/prompt.ts:resolveTools()` |
| `tool_choice` | 不发送 (正常对话) | 见下方说明 |

> `providerOptions` 是 AI SDK 框架参数 (`transform.ts:1164` 返回 `{"mimo": {}}`)，最终 HTTP 请求体中**不出现**此字段。

`tool_choice` 说明：调用方 `prompt.ts:3011/3174` 在正常步骤传入 `undefined` → AI SDK `openai-compatible` 检测到 `toolChoice == null` 后设 `openaiToolChoice = void 0` → `JSON.stringify` 忽略 `undefined` 值 → **正常对话的 HTTP body 中不发送此字段**。仅以下特殊情况才会发送：
  - `isLastStep` → `"none"`（禁止工具调用）
  - `format.type === "json_schema"` → `"required"`（结构化输出必须调工具）
...
`system` 数组由 `session/llm.ts:buildSystemArray()` + `session/llm-request-prefix.ts:buildLLMRequestPrefix()` 构建。`buildSystemArray()` 将所有内容合并为 **2 条** system 消息:

##### System Message 1: 基础 System Prompt + 环境信息 + Skills + Instructions

代码: `session/llm.ts:243-253` — 将 agent prompt、env、skills、instructions 全部 `.join("\n")` 合并为一个字符串:

1. `SystemPrompt.provider(model)` → `PROMPT_DEFAULT` (`session/prompt/default.txt`)
2. `sys.environment(model, now)` → 环境描述 (`session/system.ts:50-68`)
3. `sys.skills(ag)` → 可用技能列表 (`session/system.ts:73-85`)
4. `instruction.system()` → 自定义指令 (`session/llm-request-prefix.ts:271`)
5. `input.system` → 调用方传入的额外 system 字符串
6. `user.system` → 用户消息中携带的 system 覆盖

##### System Message 2: Memory 系统指令

代码: `session/llm.ts:266-280` (`buildMemoryInstructions()`)
...
**MiMo Code 使用 Bun 内置 `fetch()` 发送 HTTP 请求**（mimo-free 扩展中直接调用 `fetch(Z, {...G, headers: j(G, $)})`）。Bun 的 `fetch()` 实现基于其自有 HTTP 栈，与 Python `requests`/`urllib3`、Node.js `undici`、curl 等库的 HTTP 行为不同，具体差异包括但不限于：

- TLS 握手指纹（JA3/JA4）
- HTTP header 大小写和顺序
- HTTP/2 帧行为
- `Connection` / `Accept-Encoding` 等协议层 header 的默认值

**以下字段并非业务代码显式设置，而是 Bun `fetch()` 自动附加**：

| Header | 来源 |
|--------|------|
| `Host` | HTTP/1.1 协议要求，Bun 根据目标 URL 自动填充 |
| `Content-Length` | POST 请求 body 有内容时自动计算 |
| `Connection` | Bun HTTP/1.1 默认行为 |
| `Accept-Encoding` | Bun 默认值 |

**无法从代码分析确定服务端是否校验 HTTP 指纹**。建议实际测试：用 Python 带正确的业务参数发请求，能通则无需额外处理；被拒则需要进一步模拟 Bun 的 HTTP 行为（例如使用 Bun 或 curl 作为 HTTP 后端）。
