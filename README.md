...
利用 [MiMo Code](https://github.com/XiaomiMiMo/MiMo-Code) 的免费 `mimo-auto` API，转换为 OpenAI 兼容接口。单端口对外，多代理出口轮询负载均衡，每个出口独立指纹 + JWT。
...
35 个测试，覆盖配置加载、指纹生成、JWT 刷新、轮询分发、错误回退、端到端 HTTP。
